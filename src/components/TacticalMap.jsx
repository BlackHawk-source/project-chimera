...
export default function TacticalMap() {
  ...
  const reactorAnimationRef = useRef(null); 
  ...
  // Reactor Particle Animation Loop
  useEffect(() => {
    ...
    const runReactorLoop = () => {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(canvas.width / 2, canvas.height / 2, 18 + Math.sin(Date.now() / 100) * 2, 0, 2 * Math.PI);
      ctx.stroke();
      ctx.restore();

      ctx.fillStyle = pA.iconColor;
      ctx.beginPath();
      ctx.arc(40, canvas.height / 2, 10, 0, 2 * Math.PI);
      ctx.fill();

      ctx.fillStyle = pB.iconColor;
      ctx.beginPath();
      ctx.arc(canvas.width - 40, canvas.height / 2, 10, 0, 2 * Math.PI);
      ctx.fill();
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tactical_paths' }, (payload) => {
        if (payload.eventType === 'INSERT' && payload.new.theater_id === activeTheater.id) {
          // Safeguard parse incoming real-time socket strings
          let parsed = payload.new.points;
          if (typeof parsed === 'string') {
            try { parsed = JSON.parse(parsed); } catch(e) { parsed = []; }
          }
          const incomingLine = { id: payload.new.id, points: parsed };
  ...
  const fetchCloudData = async () => {
    ...
    if (pathRes.data) {
      setLines(pathRes.data.map(row => {
        // Safeguard parse old database string ghost data
        let parsed = row.points;
        if (typeof parsed === 'string') {
          try { parsed = JSON.parse(parsed); } catch(e) { parsed = []; }
        }
        return { id: row.id, points: parsed };
      }));
    }
  };
  ...
  const handleEndAction = async () => {
    ...
    if (toolMode === 'draw') {
      if (currentLine.length > 1) {
        ...
        if (data) {
          // Safeguard parse fresh return data
          let parsed = data[0].points;
          if (typeof parsed === 'string') {
            try { parsed = JSON.parse(parsed); } catch(e) { parsed = []; }
          }
          setLines([...lines, { id: data[0].id, points: parsed }]);
        }
      }
      setCurrentLine([]);
    } else if (draggedNodeId) {
      const targetNode = markers.find(m => m.id === draggedNodeId);
      if (targetNode) {
        await supabase.from('tactical_nodes').update({ x: targetNode.x, y: targetNode.y }).eq('id', draggedNodeId);
      }
    }
    setIsDragging(false); 
  ...
  const getXYAtProgress = (points, progress) => {
    ...
    const segmentStart = pts[targetSegment]; 
    const segmentEnd = pts[targetSegment + 1];
    const segmentProgress = (progress * totalSegments) - targetSegment;
    return { x: segmentStart.x + (segmentEnd.x - segmentStart.x) * segmentProgress, y: segmentStart.y + (segmentEnd.y - segmentStart.y) * segmentProgress };
  };
  ...
  const drawCanvasMatrix = () => {
    ...
    if (showHeatmap) {
      ...
      globalHeatmapPoints.forEach((pt) => {
        ...
        ctx.beginPath(); 
        ctx.arc(pt.x, pt.y, 40, 0, 2 * Math.PI); 
        ctx.fill();
      });
      ctx.restore();
    }
    ...
    lines.forEach(line => { 
      // Safeguard strictly ensure points is an array before drawing
    ...
    if (isSimulating && lines.length > 0) { 
      dynamicMovingHeroPos = getXYAtProgress(lines[lines.length - 1].points, simProgress); 
    }
